...
const StoreReviews = () => {
  ...
  // Phân loại nhãn cho từng mức sao
  ...
  return (
    <div className="card border-0 shadow-sm rounded-4 mt-5 p-4">
      <h4 className="fw-bold text-primary mb-4"><i className="bi bi-star-fill me-2 text-warning"></i> Đánh giá từ Khách hàng</h4>
      
      {/* Form Viết Đánh Giá */}
      <div className="bg-light p-4 rounded-4 mb-4">
        <h6 className="fw-bold mb-3">Gửi nhận xét của bạn về cửa hàng</h6>
        <form onSubmit={handleSubmit}>
          <div className="d-flex align-items-center mb-3">
            <div className="me-3" onMouseLeave={() => setHoverRating(0)}>
              {[1, 2, 3, 4, 5].map((star) => (
                <i key={star} 
                   className={`bi fs-3 ${star <= (hoverRating || rating) ? 'bi-star-fill text-warning' : 'bi-star text-secondary'}`}
            ...
            <span className="badge bg-primary px-3 py-2">{labels[hoverRating || rating]}</span>
          </div>
          ...
          <button type="submit" className="btn btn-primary fw-bold rounded-pill px-4" disabled={isSubmitting || !user}>
      ...
      <div className="review-list mt-2">
        ...
        {reviews.map((rev) => (
          <div key={rev._id} className="d-flex mb-4 border-bottom pb-3">
            <img src={rev.user?.avatar || 'https://cdn-icons-png.flaticon.com/512/149/149071.png'} alt="Avatar" className="rounded-circle me-3 border" style={{ width: '50px', height: '50px', objectFit: 'cover' }} />
            <div className="flex-grow-1">
              <div className="d-flex justify-content-between align-items-center">
                <h6 className="fw-bold m-0">{rev.user?.name || 'Khách hàng Ẩn danh'}</h6>
              ...
              <p className="m-0 text-dark" style={{ whiteSpace: 'pre-wrap' }}>{rev.comment}</p>
            </div>
          </div>
        ))}
      </div>
