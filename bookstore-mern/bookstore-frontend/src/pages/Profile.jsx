  ...
  const handleImageChange = async (e) => {
    ...
    if (file) {
      ...
      if (file.size > 50 * 1024 * 1024) {
        return Swal.fire(
          "Lỗi",
          "Vui lòng chọn ảnh có kích thước nhỏ hơn 50MB",
          "warning",
        );
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        "http://192.168.1.12:5000/api/auth/profile",
      ...
      if (!error.response) {
        setMessage({
          type: "danger",
          text: "Không thể kết nối tới Server. Dữ liệu ảnh có thể quá lớn làm Server ngắt kết nối!",
        });
      } else if (error.response.status === 413) {
        setMessage({
          type: "danger",
          text: "Dữ liệu ảnh quá lớn! Vượt quá giới hạn cho phép của Server.",
        });
      } else {
        setMessage({
          type: "danger",
          text:
            error.response?.data?.message || "Lỗi cập nhật! Đã có lỗi xảy ra.",
        });
      }
    } finally {
      ...
      <div className="col-lg-8">
        <div className="card border-0 shadow-sm rounded-4 overflow-hidden">
          <div
            ...
            style={{ background: "linear-gradient(45deg, #0ea5e9, #3b82f6)" }}
          ></div>
          ...
          <div className="card-body p-4 position-relative">
            <div className="text-center position-absolute top-0 start-50 translate-middle">
              <div className="bg-white p-1 rounded-circle shadow position-relative">
                {avatar ? (
                  ...
                  <div
                    className="bg-secondary rounded-circle d-flex align-items-center justify-content-center text-white fw-bold fs-1 border"
                    style={{ width: "110px", height: "110px" }}
                  >
                ...
                <label
                  ...
                  className="position-absolute bottom-0 end-0 bg-dark text-white rounded-circle p-2 shadow-sm"
                  style={{
                    cursor: "pointer",
            ...
            {user.role !== "admin" && (
              <div className="text-center mb-4">
                <Link
                  to="/reviews"
                  className="btn btn-warning fw-bold shadow-sm rounded-pill px-4 text-dark transition-hover"
                >
            ...
            {message.text && (
              <div className={`alert alert-${message.type} rounded-3 py-2`}>
                {message.text}
              </div>
            )}
              ...
              <div className="row g-3">
                ...
                <div className="col-12">
                  <label className="form-label fw-bold mt-2">
                    Đổi mật khẩu mới (Bỏ trống nếu không muốn đổi)
                  </label>
                  <input
                    type="password"
                    className="form-control rounded-3"
